        ...
        for (const term in terms) {
            ...
            do {
                ...
                if (offsetIndex > -1) {
                    ...
                    const lastLineBreakIndex: number = fileContent
                    ...
                    const nextLineBreakIndex: number =
                        fileContent.substring(termIndex).indexOf(lineBreak) + termIndex
                    // tabs are equivalent to 4 characters by default, so we must count them
                    const tabsInLine: number = (
                        fileContent
                    ...
                    diagnostics.push({
                        lineNumber:
                            (fileContent.substring(0, termIndex).match(/\n|\n\r|\r/g) || [])
                                .length + 1,
                        term: term,
